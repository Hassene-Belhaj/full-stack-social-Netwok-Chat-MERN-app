 ...
 const handleCreatePost =async(e) => {
  ...
  return (
       <Container $position='absolute' $inset='0' $bg='rgba(0,0,0,0.8)' >
            <Wrapper $width='620px' $display='flex' $fd='column' $margin='auto' $position='absolute' $top='50%' $left='50%' $transform='translate(-50%,-50%)'  >
                
                    <Title3 $padding='0 0 .3rem 0' $fw='500' $ta='center' $bg='transparent' $color='#f1f1f1'>New Thread</Title3>
                    ...
                    <DivMenu $br='25px' $padding='0rem' > 
                        <Section  ref={RefModal} $height='100%' >
                                <FlexContainer >
                                    <Section  $padding='2rem' $width='100%' $margin='auto' $display='flex' $ai='center' $gap='1rem'>
                                        <Div $width='4rem' $height='4rem'>
                                           <Image $width='100%' $height='100%' src={profilePic || '/user.jpg'} $br='50%' $objectfit='cover' />
                                        </Div>
                                        <Div>
                            ...
                            {imagePost && (
                                <Div> 
                                <Button  onClick={()=>setImagePost('')} $width='1.5rem' $height='1.5rem' $br='50%' $border='none' $outline='none' $display='flex' $jc='center' $ai='center'>
                            ...
                            {imagePost && (
                                        <Div $padding='1rem 2rem' $display='flex' $jc='center' $width='100%' $margin='auto' >
                                            <Image $width='100%' $height='18rem' $objectfit='cover' src={imagePost} $br='15px' />
                                       </Div>   
                            ...
                            <Form $display='flex' $fd='column' onSubmit={handleCreatePost}>
                                <TextArea $width='90%' $padding='1rem' $height='10rem' $bg='transparent' $outline='none' $borderRadius='15px' $resize='none'
                                name='post' value={postMessage} onChange={handleChangePostMessage} placeholder='Start A New Thread'/>
                            ...
                            <Div $padding='1rem' $display='flex' $gap='2rem'>
                                <ButtonTheme2  onClick={()=>setShowModal(false)} $padding='.5rem 2rem'>Close</ButtonTheme2>
                            <ButtonTheme1 type='submit' $padding='.5rem 2rem'>{loading ? <Spinner2 width={'1.5rem'} height={'1.5rem'} /> : 'Submit'}</ButtonTheme1>
